...
def check_filename_convention(filename):
    """Files should be named 'YYYY-MM-DD-YYY-MM-DD.csv' where the first date is the end of the time period
    and the second is the start of the time period. This checks for any dates named inconsistently.
    
    This will through an error if formatting is incorrect, and warn if the dates are not in the correct order.
...
def create_dataframe_from_dir(directory):
    ...
    for filename in file_list:
        ...
        try:
            df[CONST_COL_NAME_DATE] = pd.to_datetime(df[CONST_COL_NAME_DATE], format="%m/%d/%Y")
        except:
            try:
                df[CONST_COL_NAME_DATE] = pd.to_datetime(df[CONST_COL_NAME_DATE], format="%d/%m/%Y")
            except:
                df[CONST_COL_NAME_DATE] = pd.to_datetime(df[CONST_COL_NAME_DATE], format="%Y-%m-%d")
...
def create_dataframe(base_dir):
    """
    Given a directory that contains some other directories (ideally ordered by date when sorted by name),
    loop through all these directories, create a data frame for each, and concatenate them using
    the concat_dataframes method (which will remove duplicates keeping the latest).
    """

    if not os.path.exists(base_dir):
        return pd.DataFrame()

    df_list = [create_dataframe_from_dir(os.path.join(base_dir, directory)) for directory in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, directory))]
...
def check_missing_data(df, date_from, date_to, ignore_from_today = True):
    ...
    date_range = np.arange(date_from, date_to, dtype='datetime64[D]').astype('datetime64[ns]')
