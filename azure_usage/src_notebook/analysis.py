...
def get_ideal_avail_spnsr_costs(
    ...
    # number of days/months left (no usage registered yet) during the
    # analysis period
    ...
    # Iterates through the months of the analysis period and estinates ideal
    # and available costs. Ideal costs is the spnsr_budget equally distributed
    # over the months of the analysis period. Available costs are unspent
    # budget distributed equally over the future months (months withough
    # registered usage)
...
def get_detailed_spnsr_analysis_df(data_path, date_from, date_to):
    ...
    pivot_df.loc[
        pivot_df.SubscriptionGuid == dsg_guid, CONST_COL_NAME_SUB
    ] = dsg_name
    pivot_df.loc[
        pivot_df.SubscriptionGuid == sub_id_2, CONST_COL_NAME_SUB
    ] = sub_name_2
    pivot_df.loc[
        pivot_df.SubscriptionGuid == sub_id_3, CONST_COL_NAME_SUB
    ] = sub_name_3
    pivot_df.loc[
        pivot_df.SubscriptionGuid == sub_id_4, CONST_COL_NAME_SUB
    ] = sub_name_4
    pivot_df.loc[
        pivot_df.SubscriptionGuid == "All", CONST_COL_NAME_SUB
    ] = "Total"
...
def get_data_excl_subid(raw_data, sub_ids):
    """
    Filters data excluding particular subscription id or a list of
        subscription ids
...
def agg_subs_with_total_usage(df, total_from, total_to, sub_id, sub_name):
    ...
    Args:
        ...
        sub_name - a name of subscriptions group
    Returns:
        sub_ids - a list of subscription IDs with total costs within
            a set range
        res_df - a resulting dataframe
    """

    df_gr = (
        df.groupby([CONST_COL_NAME_SGUID])[CONST_COL_NAME_COST]
        .sum()
        .reset_index()
    )
...
def get_sub_latest_name(raw_data_df, sub_id):
    """
    Returns the latest subscription name from the raw data for a particular
        subscription id
