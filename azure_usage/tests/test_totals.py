...
def test_group_day():
    ...
    raw_usage = raw_usage[
        (raw_usage[CONST_COL_NAME_DATE] >= dt_from)
        & (raw_usage[CONST_COL_NAME_DATE] <= dt_to)
    ]
...
def test_group_year_month():
    ...
    assert (
        round(
            float(
                result[result[CONST_COL_NAME_YM] == "2019-07"][
                    CONST_COL_NAME_COST
                ]
            ),
            10,
...
def test_group_sub_year_month():
    ...
    assert (
        round(
            float(
                result[
                    (result[CONST_COL_NAME_YM] == "2016-10")
                    & (
                        result[CONST_COL_NAME_SGUID]
                        == "{" + CONST_TEST_DIR_1_SUB_ID_1 + "}"
                    )
                ][CONST_COL_NAME_COST]
            ),
