...
def test_get_data_for_subid():
    ...
    sub_ids = prep_sub_ids(
        CONST_TEST_DIR_1_SUB_ID_1 + "," + CONST_TEST_DIR_1_SUB_ID_2
    )
    result = get_data_for_subid(raw_usage, sub_ids)
...
def test_get_top_services_grouping():
    ...
    top_services = get_top_services(
        raw_usage, top_services_num=20, top_services_grp_md=CONST_RB_VALUE_0
    )

    value = round(
        float(
            top_services[
                top_services[CONST_COL_NAME_SERVICE] == "BASIC.A0 VM"
            ][CONST_COL_NAME_COST]
        ),
        10,
    )
    ...
    # checking grouping 1
    top_services = get_top_services(
        raw_usage, top_services_num=20, top_services_grp_md=CONST_RB_VALUE_1
    )
    ...
    value = round(
        float(
            top_services[
                top_services[CONST_COL_NAME_SERVICE]
                == "Virtual Machines: BASIC.A0 VM: Compute Hours"
            ][CONST_COL_NAME_COST]
        ),
        10,
    )
    ...
    # checking grouping 2
    top_services = get_top_services(
        raw_usage, top_services_num=20, top_services_grp_md=CONST_RB_VALUE_2
    )
...
def test_get_top_services():
    ...
    top_services = get_top_services(
        raw_usage, top_services_grp_md=CONST_RB_VALUE_1
    )
    ...
    raw_usage = create_dataframe(data_path)
    top_services = get_top_services(
        raw_usage, top_services_num=10, top_services_grp_md=CONST_RB_VALUE_1
    )
    ...
    value = float(
        top_services.loc[
            top_services[CONST_COL_NAME_SERVICE].str.startswith(
                CONST_NAME_OTHER
            ),
            CONST_COL_NAME_COST,
        ]
...
def test_calc_top_services_perc():
    ...
    test_value = 2 * pi
    assert round(top_services[CONST_COL_NAME_ANGLE].sum(), 13) == round(
        test_value, 13
    )
