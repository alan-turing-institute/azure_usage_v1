...
def test_parse_url_params():
    ...
    assert result_dict[URL_PARAM_REPORT] == test_repor_title.decode(
        CONST_ENCODING
    )
    assert result_dict[URL_PARAM_SUB_IDS] == test_sub_id.decode(CONST_ENCODING)
    assert result_dict[URL_PARAM_DT_FROM] == pd.to_datetime(
        test_date_from.decode(CONST_ENCODING), format="%Y-%m-%d"
    )
    assert result_dict[URL_PARAM_DT_TO] == pd.to_datetime(
        test_date_to.decode(CONST_ENCODING), format="%Y-%m-%d"
    )
    ...
    assert result_dict[URL_PARAM_DT_FROM] is None
    assert result_dict[URL_PARAM_DT_TO] is None
...
def test_read_timestamp():
    ...
    path_to_timestamp = os.path.join(
        CONST_TEST_DIR_DATA_LOADER, TIMESTAMP_FILE
    )
    result = read_timestamp(path_to_timestamp)
    assert result == pd.to_datetime("2019-12-12 15:49:30.988079")

    path_to_timestamp = os.path.join(
        CONST_TEST_DIR_DATA_LOADER, "doesnt_exist.txt"
    )
    result = read_timestamp(path_to_timestamp)
