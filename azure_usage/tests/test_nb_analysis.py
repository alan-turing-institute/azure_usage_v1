...
def test_get_ideal_avail_spnsr_costs():
    ...
    result_df, max_reg_cost_day = get_ideal_avail_spnsr_costs(raw_usage, 
        date_from, date_to, spnsr_budget, use_days = False)
...
def test_analyse_spnsr_usage():
    ...
    spnsr_remain, spnsr_usage, spnsr_usage_df, max_reg_cost_day = analyse_spnsr_usage(data_path, date_from, date_to, spnsr_budget)
...
def test_create_cost_avail_ideal_df():
    ...
    ea_usage = None
    ea_df = create_cost_avail_ideal_df(budget, ea_usage, dt_from, dt_to, dt_now)
...
def test_analyse_ea_usage():
    ...
    ea_budget, ea_usage, ea_remain, ea_df = analyse_ea_usage(budgets, ea_data_df, dt_from, dt_to, dt_now)
    print(ea_df)
