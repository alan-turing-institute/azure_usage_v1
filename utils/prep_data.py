''' 
A script to prepare a single Azure raw usage file by combining multiple files and removing duplicates.
'''
...
def setup():
    ...
    parser = argparse.ArgumentParser(description='Prepare raw Azure usage data.')
    parser.add_argument("-i", "--input", help="Path to the input files", required=True)
...
if __name__ == "__main__":
    ...
    output_file_name = '{year_end}-{month_end:02d}-{day_end:02d}'.format(year_end=dt_end.year, month_end=dt_end.month, day_end=dt_end.day)
    output_file_name = '{}-{year_st}-{month_st:02d}-{day_st:02d}.csv'.format(output_file_name, year_st=dt_st.year, month_st=dt_st.month, day_st=dt_st.day)
